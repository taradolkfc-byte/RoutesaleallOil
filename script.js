...
function buildPumpPlanRows(pumpRows, repairRows, marketRows) {
  ...
  selectedPumps.forEach(pump => {
    ...
    // ใช้รูปแบบจัดเส้นทางจากวันปกติสาย 55/65 เป็นแกนหลัก แต่ยังคงจุดปรับปรุงปั๊มเป็นจุดที่ 1
    const pumpOrderBuilder = rows => {
      const pivotPump = rows.find(isPumpRow) || pump;
      const rest = removeSameStop(rows, pivotPump);
      const pivot = validCoord(pivotPump)
        ? { lat: toNumber(pivotPump.lat), lng: toNumber(pivotPump.lng) }
        : start;
      const normalLike = orderNormalMarketRoute(pivot, rest);
      const loopRest = applyNormalRouteFieldFeedback(pivot, normalLike, rest);
      return [pivotPump, ...loopRest];
    };
...
function isTargetNormalLoopMeter(points) {
  // ใช้ต้นแบบการจัดเส้นทางจากโหมดวันปกติ/ออกตลาดทั่วไปของสาย 55 และ 65
  // กับทุกสายที่มีจุดพิกัดเพียงพอ ไม่จำกัดเฉพาะบางสายอีกต่อไป
  const valid = (points || []).filter(validCoord);
...
function improveTargetNormalLoopRoute(start, order, sourcePoints) {
  // ปรับทุกสายให้ใช้แนวคิดเดียวกับสาย 55/65: วนเป็นวง, ลดการย้อนกลับ, เลือกหัววงที่เหมาะที่สุด
  ...
  // ยอมรับเส้นทางที่เป็นวงและลดการวกกลับ แม้ระยะประมาณจะต่างจากเดิมเล็กน้อย
  return newScore <= oldScore * 1.10 ? best : order;
}
...
function buildNormalPlanRows(marketRows, planDays) {
  ...
  groups.forEach((list, key) => {
    ...
    const maxItems = Math.min(list.length, planDays * MAX_ROUTE_CUSTOMER_STOPS);
    const usable = list.slice(0, maxItems);
    for (let dayIndex = 0; dayIndex < planDays; dayIndex++) {
      const chunk = usable.slice(dayIndex * MAX_ROUTE_CUSTOMER_STOPS, (dayIndex + 1) * MAX_ROUTE_CUSTOMER_STOPS);
...
function buildRepairPlanRows(repairRows, marketRows, planDays) {
  ...
  candidates.forEach((repair, repairIndex) => {
    ...
    // ตารางซ่อมใช้ template วงกลมแบบเดียวกับวันปกติสาย 55/65 แต่ล็อกจุดซ่อมให้อยู่ในแผนเสมอ
    const repairOrderBuilder = rows => {
      const normalLike = orderNormalMarketRoute(start, rows);
      return applyNormalRouteFieldFeedback(start, normalLike, rows);
    };
